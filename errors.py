class LexicalError(Exception):
    ...
    def formato(self) -> str:
        result = (f"Linha {self.linha}, Coluna {self.coluna}: " f"{self.mensagem}")
...
def erro(tipoErro: int, charVal: str, linha: int, coluna: int, linhaCodigo: str):
    if tipoErro == 0: mensagem = caractereInesperado(charVal)
    elif tipoErro == 1: mensagem = stringNaoFechada()
    elif tipoErro == 2: mensagem = numeroMalFormado(charVal)
    else: mensagem = "Erro léxico desconhecido"
